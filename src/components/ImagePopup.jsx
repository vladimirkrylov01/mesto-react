...
const ImagePopup = ({...props}) => {
  ...
  return (
    <section className={props.active ?
      ...
      >
        <button className="popup__button-close popup-type-image__button-close"
                type="button" aria-label="Закрыть" onClick={props.onClose}>
        </button>
        <figure className="popup__figure">
          {props.card.link && <img className="popup__image"
               src= {props.card.link}
               alt= {props.card.name}
               onLoad={e => {
